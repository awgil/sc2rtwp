...
public:
	...
	static void stack()
	{
		if (auto& inst = instance())
		{
			...
			for (int i = 0; i < numFrames; ++i)
			{
				...
				log("> [{}] 0x{:016X} ({} + {:#x})", i, reinterpret_cast<u64>(stack[i]), std::string_view{ buf, fnLen }, static_cast<char*>(stack[i]) - static_cast<char*>(mi.AllocationBase));
			}
		}
	}
	...
	{
		log("{}: {:08X} @ {} (thread {})", tag, info->ExceptionRecord->ExceptionCode, info->ExceptionRecord->ExceptionAddress, GetCurrentThreadId());
		for (auto i = 0; i < info->ExceptionRecord->NumberParameters; ++i)
			log("param [{}]: {:016X}", i, info->ExceptionRecord->ExceptionInformation[i]);
		log("rax: {:016X}, rcx: {:016X}, rdx: {:016X}, rbx: {:016X}, rsp: {:016X}, rbp: {:016X}, rsi: {:016X}, rdi: {:016X}",
		...
		log(" r8: {:016X},  r9: {:016X}, r10: {:016X}, r11: {:016X}, r12: {:016X}, r13: {:016X}, r14: {:016X}, r15: {:016X}",
...
private:
	...
	Logger() : mPipe(R"(\\.\pipe\sc2rtwp_log)") {}
